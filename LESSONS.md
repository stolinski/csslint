...
- CLI integration tests are more portable when they fall back to a built `target/debug/clint` binary if `CARGO_BIN_EXE_clint` is unavailable, so exit-code and snapshot assertions still run in package-scoped test commands.
