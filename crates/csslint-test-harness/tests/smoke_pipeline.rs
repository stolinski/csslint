...
fn emits_no_empty_rules_diagnostic_for_empty_block() {
    ...
    let extracted = csslint_extractor::extract_styles(file_id, Path::new("fixture.css"), ".btn {}");
    let parsed = csslint_parser::parse_style(&extracted[0]).expect("parser should accept non-empty style");
